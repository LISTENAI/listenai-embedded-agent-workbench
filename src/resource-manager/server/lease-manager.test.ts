// @ts-expect-error - vi, beforeEach, afterEach work at runtime but verbatimModuleSyntax + vitest re-exports cause false TS2305
...
describe("LeaseManager", () => {
  ...
  describe("scanExpired", () => {
    ...
    it("expires lease after timeout and calls callback", () => {
      ...
      // lease should be removed after scan
      expect(lm.getLease(leaseId)).toBeUndefined();
    });

    it("does NOT expire recently-refreshed lease", () => {
      const lm = new LeaseManager();
      const leaseId = lm.createLease("dev1", "skill1");
  ...
  describe("custom timeoutMs", () => {
    it("uses custom timeout for expiration", () => {
