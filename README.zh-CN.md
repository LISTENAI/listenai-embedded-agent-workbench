...
- `packages/skill-logic-analyzer` - `@listenai/skill-logic-analyzer` package，导出打包后的 logic-analyzer skill boundary、request/result contracts、capture loading helpers，以及 waveform-analysis surface。
...
根目录下的 `src/index.ts` 是一个轻量兼容层，用于重新导出由各 workspace 拥有的公共 API。它不是主要实现位置；真正由 package 拥有的代码位于 `packages/` 和 `share/` 下。
...
- `skills/logic-analyzer/README.md` - 面向 host 的打包后 logic-analyzer API 与生命周期说明。
